#pragma ident "$Id: tocss.c,v 1.3 2005/10/11 22:45:17 dechavez Exp $"
/*======================================================================
 * 
 ...
 *  created is always named xfer.wfdisc.
 *
...
#define WFDISC_PREFIX "xfer"
#define WFDISC_FNAME WFDISC_PREFIX".wfdisc"
...
static char *fid = "Xfer_ToCSS:finish";
    ...
    cssioWriteDescriptor(".", WFDISC_PREFIX);
...
static char *fid = "Xfer_ToCSS:wfdisc";
    ...
    wfdisc.smprate = (float) (1.0 / packet->sint);
...
int Xfer_ToCSS(FILE *fp, int dupflag, int swap)
{
int i, j, status;
u_long order;
long count = 0;
...
static char *fid = "Xfer_ToCSS";
    ...
    datatype = cssio_datatype(sizeof(long), order, 1);
    ...
    while (Xfer_RdPacket(fp, &packet) == 0) {
        if (swap) util_lswap(packet.data, packet.nsamp);
 ...
 *
 * $Log: tocss.c,v $
 * Revision 1.3  2005/10/11 22:45:17  dechavez
 * write descriptor file
 *
 * Revision 1.2  2005/05/25 22:41:56  dechavez
