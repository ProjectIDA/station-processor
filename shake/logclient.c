...
char *q330SeedSend(void *seed_record)
{
  ...
  if (iSocket < 0)
  {
        ...
        sizeof(serv_addr)) < 0)
    {
      close(iSocket);
      iSocket = -1;
      return "Unable to connect to archd";
    }
  ...
  {
    sprintf(errmsgbuf, "recv archd failed, errno %d %s",
      errno, strerror(errno));
    close(iSocket);
    iSocket = -1;
    return errmsgbuf;
...
{
  char  *errmsg;
  char  seedRecord[MAX_RECORD_SIZE];    // Buffer to store seed record
