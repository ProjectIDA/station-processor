#pragma ident "$Id: sint.c,v 1.4 2007/01/11 21:59:25 dechavez Exp $"
/*======================================================================
 *
 * Lookup sample interval from sint table.
 *
 *====================================================================*/
#include "isi/db.h"

static BOOL FlatFileLookup(DBIO *db, ISI_STREAM_NAME *name, int *sint)
{
IDAFF_SINTMAP smap;
...
static BOOL MysqlLookup(DBIO *db, ISI_STREAM_NAME *name, int *sint)
{
#ifdef HAVE_MYSQL
char query[1024];
MYSQL_RES *result;
...
/* Look first for a station specific entry */
    ...
    query[0] = 0;
    strcat(query, "SELECT sint FROM sint ");
...
{
LOGIO *lp;
static char *fid = "isidbLookupSint";

    if (db == NULL || name == NULL || sint == NULL) {
    ...
    if (db->engine != DBIO_MYSQL) {
        return FlatFileLookup(db, name, sint);
    } else {
        return MysqlLookup(db, name, sint);
    }
}
