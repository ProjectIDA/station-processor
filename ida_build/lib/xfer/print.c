#pragma ident "$Id: print.c,v 1.4 2006/06/26 22:15:31 dechavez Exp $"
/*======================================================================
 *
...
static void xfer_PrintCnfGen1(FILE *fp, struct xfer_cnfgen1 *cnf)
{
int i, j;
...
#ifdef NRTS_SUPPORT
static void xfer_PrintCnfNrts(FILE *fp, struct xfer_cnfnrts *cnf)
{
int i, j;
    ...
    for (i = 0; i < cnf->nsta; i++) {
        ...
        for (j = 0; j < cnf->sta[i].nchn; j++) {
            ...
            if (cnf->sta[i].chn[j].type == NRTS_IDA) {
                fprintf(fp, "(IDA)\n");
            } else if (cnf->sta[i].chn[j].type == NRTS_IDA10) {
                fprintf(fp, "(IDA10)\n");
            } else if (cnf->sta[i].chn[j].type == NRTS_ASL) {
                fprintf(fp, "(ASL)\n");
            } else {
...
static void xfer_PrintWavGen1(FILE *fp, struct xfer_wavgen1 *wav)
{
    fprintf(fp, "\n");
...
#ifdef IDA_SUPPORT
static void xfer_PrintWavIda(FILE *fp, struct xfer_wavida *wav)
{
...
#ifdef SEED_SUPPORT
static void xfer_PrintWavSeed(FILE *fp, struct xfer_wavseed *wav)
{
 ...
 *
 * $Log: print.c,v $
 * Revision 1.4  2006/06/26 22:15:31  dechavez
 * removed unreferenced local variables
 *
 * Revision 1.3  2006/02/09 20:01:48  dechavez
 * IDA handle support
 *
