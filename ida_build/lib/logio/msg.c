#pragma ident "$Id: msg.c,v 1.11 2009/01/05 17:37:58 dechavez Exp $"
/*======================================================================
 *
...
char *logioTstamp(char *tsbuf)
{
time_t now;
static char mtunsafe[LOGIO_MAX_TSLEN+1];

    if (tsbuf == NULL) tsbuf = mtunsafe;
    now = time(NULL);
    strftime(tsbuf, LOGIO_MAX_TSLEN, LOGIO_DEF_TFMT, localtime(&now));
    return tsbuf;
}
...
FILE *fp;
time_t now;
char timestamp[LOGIO_MAX_TSLEN+1];
...
/* timestamp */
    ...
    fprintf(fp, "%s ", logioTstamp(timestamp));
 ...
 *
 * $Log: msg.c,v $
 * Revision 1.11  2009/01/05 17:37:58  dechavez
 * added logioTstamp(), changed DeliverMessage() to use same
 *
