#pragma ident "$Id: loginfo.c,v 1.2 2007/01/04 23:34:27 dechavez Exp $"
/*======================================================================
 *
 *  Log tty port parameters.
 *
 *====================================================================*/
#include "oldttyio.h"
#include "util.h"
...
void ttyio_loginfo(level, port, tty)
int  level;
char *port;
struct termios *tty;
{
...
static struct flagmap ffdlymap[NFFDLY] = {
    ...
    util_log(level, "device      = %s", port);
    util_log(level, "ibaud       = %ld", (long) ttyio_cfgetispeed(tty));
    util_log(level, "obaud       = %ld", (long) ttyio_cfgetospeed(tty));
...
#ifdef SUNOS
    util_log(level, "c_line      = 0x%04x", tty->c_line);
#endif
    util_log(level, "c_cc[VMIN]  = %d", tty->c_cc[VMIN]);
    util_log(level, "c_cc[VTIME] = %d", tty->c_cc[VTIME]);
