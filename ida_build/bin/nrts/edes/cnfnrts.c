#pragma ident "$Id: cnfnrts.c,v 1.2 2006/02/10 02:05:34 dechavez Exp $"
/*======================================================================
 *
 *  Load an XFER_CNFNRTS format structure
 *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    for (i = 0; i < edes->nsys; i++) {
        for (j = 0; j < SYS->nsta; j++) {
            ...
            for (k = 0; k < STA->nchn; k++) {
                ...
                CPTR->end     = CHN->end;
                CPTR->type    = SYS->type;
                CPTR->hlen    = CHN->hdr.len;
                CPTR->dlen    = CHN->dat.len;
                CPTR->nrec    = CHN->hdr.nrec;
                CPTR->nhide   = CHN->hdr.hide;
                CPTR->latency = edes->tstamp - CHN->tread;
            }
        }
        ++cnf->nsta;
    }
