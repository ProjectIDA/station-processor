#pragma ident "$Id: toseed.c,v 1.3 2005/05/23 21:02:25 dechavez Exp $"
/*======================================================================
 * 
...
int cmpval;
    ...
    cmpval = strcasecmp(a->fsdh.locid, b->fsdh.locid);
    if (cmpval != 0) return cmpval;
...
static char *fid = "Xfer_ToSEED:newfile";
    ...
    chanToChnLoc(packet->cname, new->info.fsdh.chnid, new->info.fsdh.locid);
...
struct file_info *info;
char chnid[CHNLEN+1], locid[LOCLEN+1];
...
/* Otherwise see if we have already encountered this sta/chn */
    ...
    while (crnt != (struct file_list *) NULL) {
        chanToChnLoc(packet->cname, chnid, locid);
        if (
            strcasecmp(packet->sname, crnt->info.fsdh.staid) == 0 &&
            strcasecmp(        chnid, crnt->info.fsdh.chnid) == 0 &&
            strcasecmp(        locid, crnt->info.fsdh.locid) == 0
        ) info = &crnt->info;
        crnt = crnt->next;
    }
...
/* Demultiplex raw data to disk */
    ...
    while (Xfer_RdPacket(ifp, &packet) == 0) {
        ...
        if (dupflag && info->prev_tofs == info->tofs) {
            util_log(2, "%s: dup %s:%s:%s ignored",
                fid, info->fsdh.staid, info->fsdh.chnid, info->fsdh.locid
            );
...
/* Process through the accumulated list of demux files */
    ...
    for (i = 0; i < nfiles; i++) {
        ...
        util_log(2, "converting %s:%s:%s file %s",
            info[i].fsdh.staid, info[i].fsdh.chnid, info[i].fsdh.locid, info[i].name
        );
 ...
 *
 * $Log: toseed.c,v $
 * Revision 1.3  2005/05/23 21:02:25  dechavez
 * cleaned up includes (05-23 update AAP)
 *
 * Revision 1.2  2002/02/13 05:27:10  dec
 * split long chan names into chan and loc
 *
