#pragma ident "$Id: modbus.c,v 1.1 2009/05/14 17:22:00 dechavez Exp $"
/*
...
#define MODBUS_UNKNOWN_ERROR_MSG "Not defined in modbus specification"
...
static void error_treat(modbus_param_t *mb_param, int code, const char *string)
{
        ...
        if (mb_param->error_handling == MODBUS_FLUSH_OR_RECONNECT_ON_ERROR) {
                switch (code) {
                ...
                case MODBUS_ILLEGAL_FUNCTION:
                        break;
                default:
                        if (mb_param->type_com == MODBUS_RTU) {
                                            ...
                                            uint8_t *query)
{
        ...
        case MODBUS_FC_READ_COIL_STATUS:
        case MODBUS_FC_READ_INPUT_STATUS: {
                                 ...
                                 uint8_t *query)
{
        ...
        return MODBUS_PRESET_QUERY_LENGTH_RTU;
}
                                 ...
                                 uint8_t *query)
{
        ...
        */
        ...
        return MODBUS_PRESET_QUERY_LENGTH_TCP;
}
...
static int build_response_basis_rtu(sft_t *sft, uint8_t *response)
{
        ...
        return MODBUS_PRESET_RESPONSE_LENGTH_RTU;
}
...
static int build_response_basis_tcp(sft_t *sft, uint8_t *response)
{
        ...
        return MODBUS_PRESET_RESPONSE_LENGTH_TCP;
}
                       ...
                       int query_length)
{
        ...
        if ((ret == -1) || (ret != query_length)) {
                ret = MODBUS_PORT_SOCKET_FAILURE;
...
static uint8_t compute_query_length_header(int function)
{
        ...
        if (function <= MODBUS_FC_FORCE_SINGLE_COIL ||
            function == MODBUS_FC_PRESET_SINGLE_REGISTER)
                /* Read and single write */
                length = 4;
        else if (function == MODBUS_FC_FORCE_MULTIPLE_COILS ||
                 function == MODBUS_FC_PRESET_MULTIPLE_REGISTERS)
                /* Multiple write */
                length = 5;
        else
                length = 0;
...
static int compute_query_length_data(modbus_param_t *mb_param, uint8_t *msg)
{
        ...
        if (function == MODBUS_FC_FORCE_MULTIPLE_COILS ||
            function == MODBUS_FC_PRESET_MULTIPLE_REGISTERS)
...
{                                                                                  \
    while ((select_ret = select(mb_param->fd+1, &rfds, NULL, NULL, &tv)) == -1) {  \
            if (errno == EINTR) {                                                  \
                    ...
                    error_treat(mb_param, MODBUS_SELECT_FAILURE, "Select failure");\
                    return MODBUS_SELECT_FAILURE;                                  \
            }                                                                      \
    }                                                                              \
                                                                                   \
    if (select_ret == 0) {                                                         \
            /* Call to error_treat is done later to manage exceptions */           \
            return MODBUS_COMM_TIME_OUT;                                           \
    }                                                                              \
}
                       ...
                       uint8_t *msg, int *p_msg_length)
{
        ...
        if (mb_param->debug) {
                if (msg_length_computed == MODBUS_MSG_LENGTH_UNDEFINED)
        ...
        if (msg_length_computed == MODBUS_MSG_LENGTH_UNDEFINED) {
                ...
                tv.tv_sec = 0;
                tv.tv_usec = MODBUS_TIME_OUT_BEGIN_OF_TRAME;
                state = COMPLETE;
        }
        ...
        while (select_ret) {
                ...
                if (read_ret == 0) {
                        ...
                        return MODBUS_CONNECTION_CLOSED;
                } else if (read_ret < 0) {
                        /* The only negative possible value is -1 */
                        error_treat(mb_param, MODBUS_PORT_SOCKET_FAILURE,
                                    "Read port/socket failure");
                        return MODBUS_PORT_SOCKET_FAILURE;
                }
                ...
                if ((*p_msg_length) < msg_length_computed) {
                        ...
                        case BYTE:
                                ...
                                msg_length_computed += length_to_read;
                                if (msg_length_computed > MODBUS_MAX_MESSAGE_LENGTH) {
                                     error_treat(mb_param, MODBUS_TOO_MANY_DATA, "Too many data");
                                     return MODBUS_TOO_MANY_DATA;  
                                }
                                state = COMPLETE;
                                break;
                        case COMPLETE:
                ...
                if (length_to_read > 0) {
                        /* If no character at the buffer wait
                           MODBUS_TIME_OUT_END_OF_TRAME before to generate an error. */
                        tv.tv_sec = 0;
                        tv.tv_usec = MODBUS_TIME_OUT_END_OF_TRAME;
                          ...
                          uint8_t *response)
{
        ...
        if (ret == 0) {
                ...
                case MODBUS_FC_READ_INPUT_REGISTERS:
                        ...
                        break;
                case MODBUS_FC_FORCE_MULTIPLE_COILS:
                case MODBUS_FC_PRESET_MULTIPLE_REGISTERS:
                        /* N Write functions */
                        query_nb_value = (query[offset+4] << 8) + query[offset+5];
                ...
                if (query_nb_value == response_nb_value) {
                        ...
                        ret = MODBUS_ILLEGAL_DATA_VALUE;
                        error_treat(mb_param, MODBUS_ILLEGAL_DATA_VALUE, s_error);
                        free(s_error);
                }
        } else if (ret == MODBUS_COMM_TIME_OUT) {
                        ...
                        if (0x80 + query[offset + 1] == response[offset + 1]) {
                                ...
                                if (exception_code < NB_TAB_ERROR_MSG) {
                                        ...
                                        sprintf(s_error,
                                                ...
                                                response[offset + 2]);
                                        error_treat(mb_param, MODBUS_INVALID_EXCEPTION_CODE,
                                                    s_error);
                                        free(s_error);
                                        return MODBUS_INVALID_EXCEPTION_CODE;
                                }
                        }
...
{                   
        ...
        if (mb_param->type_com == MODBUS_TCP)
        ...
        case MODBUS_FC_READ_COIL_STATUS: {
                ...
                if ((address + nb) > mb_mapping->nb_coil_status) {
                        ...
                        resp_length = response_exception(mb_param, &sft,
                                                         MODBUS_ILLEGAL_DATA_ADDRESS, response);  
                } else {
        ...
        case MODBUS_FC_READ_INPUT_REGISTERS: {
                ...
                break;
        case MODBUS_FC_FORCE_SINGLE_COIL:
                if (address >= mb_mapping->nb_coil_status) {
                        ...
                        if (data == 0xFF00 || data == 0x0) {
                                ...
                                printf("Illegal data value %0X in force_single_coil request at address %0X\n",
                                       data, address);
                                resp_length = response_exception(mb_param, &sft,
                                                                 MODBUS_ILLEGAL_DATA_VALUE, response);
                        }
                }
                break;          
        case MODBUS_FC_PRESET_SINGLE_REGISTER:
...
int modbus_listen(modbus_param_t *mb_param, uint8_t *query, int *query_length)
{
        ...
        ret = receive_msg(mb_param, MODBUS_MSG_LENGTH_UNDEFINED, query, query_length);
                          ...
                          int start_addr, int nb, uint8_t *data_dest)
{
        ...
        uint8_t query[MODBUS_MIN_QUERY_LENGTH];
        uint8_t response[MODBUS_MAX_MESSAGE_LENGTH];
                     ...
                     int nb, uint8_t *data_dest)
{
        ...
        if (nb > MODBUS_MAX_STATUS) {
                printf("ERROR Too many coils status requested (%d > %d)\n",
                       nb, MODBUS_MAX_STATUS);
                return MODBUS_TOO_MANY_DATA;
        }

        status = read_io_status(mb_param, slave, MODBUS_FC_READ_COIL_STATUS,
                      ...
                      int nb, uint8_t *data_dest)
{
        ...
        status = read_io_status(mb_param, slave, MODBUS_FC_READ_INPUT_STATUS,
                                start_addr, nb, data_dest);
                           ...
                           int start_addr, int nb, uint16_t *data_dest)
{
        ...
        status = read_registers(mb_param, slave, MODBUS_FC_READ_HOLDING_REGISTERS,
                                start_addr, nb, data_dest);
        return status;
}
                         ...
                         int start_addr, int nb, uint16_t *data_dest)
{
        ...
        status = read_registers(mb_param, slave, MODBUS_FC_READ_INPUT_REGISTERS,
                                start_addr, nb, data_dest);
                      ...
                      int addr, int value)
{
        ...
        if (ret > 0) {
                ...
                uint8_t response[MODBUS_MIN_QUERY_LENGTH];
                ret = modbus_receive(mb_param, query, response);
        }
                      ...
                      int coil_addr, int state)
{
        ...
        status = set_single(mb_param, slave, MODBUS_FC_FORCE_SINGLE_COIL,
                            coil_addr, state);
                           ...
                           int reg_addr, int value)
{
        ...
        status = set_single(mb_param, slave, MODBUS_FC_PRESET_SINGLE_REGISTER,
                            reg_addr, value);
                         ...
                         const uint8_t *data_src)
{
        ...
        uint8_t query[MODBUS_MAX_MESSAGE_LENGTH];
        ...
        query_length = build_query_basis(mb_param, slave,
                                         MODBUS_FC_FORCE_MULTIPLE_COILS, 
                                         start_addr, nb, query);
                              ...
                              const uint16_t *data_src)
{
        ...
        query_length = build_query_basis(mb_param, slave,
                                         MODBUS_FC_PRESET_MULTIPLE_REGISTERS, 
                                         start_addr, nb, query);
        byte_count = nb * 2;
        query[query_length++] = byte_count;
                    ...
                    uint8_t *data_dest)
{
        ...
        query_length = build_query_basis(mb_param, slave, MODBUS_FC_REPORT_SLAVE_ID, 
                                         0, 0, query);
                     ...
                     int stop_bit)
{
        ...
        mb_param->type_com = MODBUS_RTU;
        mb_param->header_length = MODBUS_HEADER_LENGTH_RTU;
        mb_param->checksum_length = MODBUS_CHECKSUM_LENGTH_RTU;
}
...
void modbus_init_tcp(modbus_param_t *mb_param, const char *ip, int port)
{
        ...
        mb_param->port = port;
        mb_param->type_com = MODBUS_TCP;
        mb_param->header_length = MODBUS_HEADER_LENGTH_TCP;
        mb_param->checksum_length = MODBUS_CHECKSUM_LENGTH_TCP;
        mb_param->error_handling = MODBUS_FLUSH_OR_RECONNECT_ON_ERROR;
}
...
{
        if (error_handling == MODBUS_FLUSH_OR_RECONNECT_ON_ERROR ||
            error_handling == MODBUS_NOP_ON_ERROR) {
 ...
 *
 * $Log: modbus.c,v $
 * Revision 1.1  2009/05/14 17:22:00  dechavez
 * initial release
 *
 */
