#pragma ident "$Id: handshake.c,v 1.7 2008/08/20 18:20:01 dechavez Exp $"
/*======================================================================
 * 
 * IACP handshake
 *
...
UINT8 *ptr;
    ...
    switch (type) {
      ...
      case IACP_TYPE_RCVSIZ:
        ...
        break;
      default:
        logioMsg(iacp->lp, LOG_WARN, "%s: skip unknown handshake parameter: type %lu, len=%lu", iacp->peer.ident, type, len);
    }
    return len;
}
...
/* Read all the parameters */
    ...
    if (count > IACP_MAX_HANDSHAKE_COUNT) {
        logioMsg(iacp->lp, LogLevel, "%s: protocol error (illegal handshake parameter count %lu)", fid, count);
        return FALSE;
    }
...
/* NRTS protocol handled elsehwere */

    if (iacp->attr.at_proto == IACP_PROTO_NRTS) return iacpNrtsClientHandshake(iacp);
...
/* NRTS protocol handled elsehwere */
    ...
    if (iacp->attr.at_proto == IACP_PROTO_NRTS) return iacpNrtsServerHandshake(iacp);
 ...
 *
 * $Log: handshake.c,v $
 * Revision 1.7  2008/08/20 18:20:01  dechavez
 * added branches to handle NRTS protocol handshakes
 *
 * Revision 1.6  2008/08/05 20:04:25  dechavez
 * check for bad paremeter counts from clients
 *
