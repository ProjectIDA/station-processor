...
public class OFCb2000
{
	...
	public OFCb2000(byte[] blockette2000)
	{
		...
		if (type == 2000)
		{
			...
			end_time = crack.m_end_time;

			if (crack.get_num_rows() > 0)
			{
				average_data = new int[crack.get_num_rows()];
				low_data = new int[crack.get_num_rows()];
				high_data = new int[crack.get_num_rows()];
			}
			for (int i=0; i < crack.get_num_rows(); i++)
			{ 
				average_data[i] = (int) crack.m_rows.get(i).m_average;
