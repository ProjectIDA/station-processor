#pragma ident "$Id: string.c,v 1.7 2008/10/02 22:55:29 dechavez Exp $"
/*======================================================================
 *
...
{
static char mt_unsafe[] = "xxx.xxx.xxx.xxx + slop";

    if (dest == NULL) dest = mt_unsafe;

    dest[0] = 0;
    sprintf(dest+strlen(dest), "%d.", (addr & 0xff000000) >> 24);
    sprintf(dest+strlen(dest), "%d.", (addr & 0x00ff0000) >> 16);
    sprintf(dest+strlen(dest), "%d.", (addr & 0x0000ff00) >>  8);
    sprintf(dest+strlen(dest), "%d",  (addr & 0x000000ff) >>  0);
 ...
 *
 * $Log: string.c,v $
 * Revision 1.7  2008/10/02 22:55:29  dechavez
 * added utilDotDecimalString
 *
