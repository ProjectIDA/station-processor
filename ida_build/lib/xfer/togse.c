#pragma ident "$Id: togse.c,v 1.3 2002/02/07 18:30:04 dec Exp $"
/*======================================================================
 *
...
static char *fid = "Xfer_ToGSE:inihdr";
    ...
    if (strlen(packet->cname) <= 3) {
        ...
        memcpy(hdr->chn, packet->cname, 3);
        hdr->chn[3] = 0;
        util_ucase(hdr->chn);
        memcpy(hdr->auxid, packet->cname + 3, 4);
        hdr->auxid[4] = 0;
...
/* Zero length files are OK, we just ignore them */
 ...
 *
 * $Log: togse.c,v $
 * Revision 1.3  2002/02/07 18:30:04  dec
 * split long channel names into chan and auxid parts
 *
 * Revision 1.2  2000/10/17 23:39:59  dec
 * deal with zero length files OK
 *
