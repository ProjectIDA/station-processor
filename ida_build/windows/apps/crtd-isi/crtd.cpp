...
//	ParseCommandLine(cmdInfo);
	...
	if (!pMainFrame->Create(lpClass,"Continuous Real-Time Display(ISI)", WS_VISIBLE | WS_OVERLAPPEDWINDOW,rcM,NULL,NULL))	return FALSE;

	m_pMainWnd = pMainFrame;
	pMainFrame->ShowWindow(m_nCmdShow|SW_SHOWMAXIMIZED);
	pMainFrame->UpdateWindow();

	NrtsInfo.LoadConfigFromIni();
	if(bAutorun) pMainFrame->PostMessage(WM_COMMAND, ID_RUN, 0);
	return TRUE;
	}
