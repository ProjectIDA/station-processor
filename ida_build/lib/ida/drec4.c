#pragma ident "$Id: drec4.c,v 1.1 2006/02/08 22:57:42 dechavez Exp $"
/*======================================================================
 *
 *  Load a rev 4 data record.
 *
 *====================================================================*/
#include "ida.h"
#include "protos.h"
...
/*  Load the header  */
    ...
    if ((retval = dhead_rev4(ida, &dest->head, src)) < 0) return retval;
...
/*  Load the data  */
    ...
    if (swab) {
        if (dest->head.form == S_UNCOMP) {
            SSWAB((short *) dest->data, dest->head.nbytes/2);
        } else if (dest->head.form == L_UNCOMP) {
            LSWAB((long *) dest->data, dest->head.nbytes/4);
        }
    }
